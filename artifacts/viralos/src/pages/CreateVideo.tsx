...
export default function CreateVideo() {
  ...
  async function handleGenerateScript() {
    ...
    setIsLoading(true);
    setLoadingLabel("AI Script Engine");
    setStage("script");

    const pid = await createProjectInDB();

    simulateSteps([
      "Initializing AI script engine — loading viral pattern library...",
      ...
      // Call real AI script generation API
      try {
      ...
      setSteps([]);
      toast({ title: "Script generated by Groq AI" });
    });
  }
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto space-y-5">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">Groq AI Script → Neural Voice → Cinematic Render → Export</p>
      </div>
            ...
            <div className="glass rounded-xl border border-border p-6 space-y-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Groq AI generates a viral script + real downloadable video</p>
                </div>
              </div>
              ...
              <Button onClick={handleGenerateScript} disabled={!title.trim() || !prompt.trim()} className="w-full gap-2">
                <Sparkles className="w-4 h-4" /> Generate Script with Groq AI <ChevronRight className="w-4 h-4" />
              </Button>
            </div>
          )}
            ...
            <div className="glass rounded-xl border border-border p-6 space-y-5">
              ...
              {isLoading && (
                <div className="space-y-4">
                  ...
                  <p className="text-xs text-center text-muted-foreground font-mono">{loadingProgress}% · Groq AI processing</p>
                </div>
              )}
